...
&n,l,orbital_label,idxrf1,idxrf2,occ)
implicit none
!
type(C_PTR), target, intent(in) :: handler
...
!> @param [in] handler Simulation context handler.
!> @param [in] label Atom type label.
subroutine sirius_atom_type_update(handler,label)
implicit none
!
type(C_PTR), target, intent(in) :: handler
character(*), target, intent(in) :: label
...
!
interface
subroutine sirius_atom_type_update_aux(handler,label)&
...
label_c_type = string_f2c(label)
label_ptr = C_LOC(label_c_type)
call sirius_atom_type_update_aux(handler_ptr,label_ptr)
deallocate(label_c_type)
